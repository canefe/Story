 ...
 */
class NPCNameManager(private val plugin: Story) {
	...
	val nameBanksDirectory: File = File(plugin.dataFolder, "namebanks").apply {
		if (!exists()) {
			mkdirs()
		}
	}
	 ...
	 */
	private fun saveNameBank(bankName: String, nameBank: NameBank) {
	 ...
	 */
	fun getOrCreateAlias(npcId: String, anchorKey: String?, nameBankName: String?, location: String?): NPCAlias {
	 ...
	 */
	private fun generateAlias(nameBank: NameBank, seed: Long, location: String?): NPCAlias {
		...
		while (attempts < maxAttempts) {
			...
			if (!isCanonicalNameTaken(canonicalName)) {
				...
				val callsign = if (nameBank.culturalMarkers.isNotEmpty() && random.nextFloat() < 0.4f) {
		...
		val callsign = if (nameBank.culturalMarkers.isNotEmpty()) {
			nameBank.culturalMarkers.random(random)
		} else {
			null
		}
	 ...
	 */
	private fun createAliasFromCanonicalName(canonicalName: String, npcId: String): NPCAlias {
		...
		val callsign = if (npcId.length > 8) {
	 ...
	 */
	private fun createDisplayHandle(givenName: String, familyName: String): String {
		return when {
	 ...
	 */
	private fun updateRecencyCache(location: String, canonicalName: String) {
	 ...
	 */
	private fun getNameBank(nameBankName: String?): NameBank? {
		return nameBankName?.let { nameBanks[it] }
	}
	...
	private fun getDefaultNameBank(): NameBank {
		return nameBanks.values.firstOrNull() ?: NameBank(
	 ...
	 */
	fun getCanonicalName(npcId: String): String? {
		return npcIdToCanonicalName[npcId]
	}

	/**
	 * Get NPC ID from canonical name
	 */
	fun getNpcId(canonicalName: String): String? {
		return canonicalNameToNpcId[canonicalName]
	}

	/**
	 * Check if an NPC has an assigned alias
	 */
	fun hasAlias(npcId: String): Boolean {
		return npcIdToCanonicalName.containsKey(npcId)
	}
	 ...
	 */
	fun getAvailableNameBanks(): Set<String> {
		return nameBanks.keys
	}
	...
	private fun isCanonicalNameTaken(canonicalName: String): Boolean {
		...
		val citizensNPCs = net.citizensnpcs.api.CitizensAPI.getNPCRegistry()
