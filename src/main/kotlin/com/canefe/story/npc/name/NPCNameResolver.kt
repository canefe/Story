 ...
 */
class NPCNameResolver(private val plugin: Story) {
	...
	fun resolveCanonicalName(npc: NPC): String {
		return resolveCanonicalNameInternal(
			npc.name,
	 ...
	 */
	fun resolveCanonicalName(entity: Entity, displayName: String): String {
		return resolveCanonicalNameInternal(displayName, entity.uniqueId.toString(), entity.location.toString())
	}

	/**
	 * Resolves canonical name with explicit parameters (used by NPCContextGenerator)
	 */
	fun resolveCanonicalName(displayName: String, entityId: String, location: String?): String {
		return resolveCanonicalNameInternal(displayName, entityId, location)
	}

	/**
	 * Core name resolution logic
	 */
	private fun resolveCanonicalNameInternal(displayName: String, entityId: String, location: String?): String {
		...
		if (existingData?.generic == true && existingData.nameBank != null) {
			...
			val alias = nameManager.getOrCreateAlias(
				npcId = entityId,
	 ...
	 */
	fun createAndReplaceNPCInConversations(originalNPC: NPC, alias: NPCNameManager.NPCAlias): NPC? {
		try {
	 ...
	 */
	private fun createCitizensNPCWithAlias(originalNPC: NPC, alias: NPCNameManager.NPCAlias): NPC? {
		try {
	 ...
	 */
	private fun copyNPCProperties(originalNPC: NPC, newNPC: NPC) {
		try {
	 ...
	 */
	private fun replaceNPCInConversation(conversation: Conversation, oldNPC: NPC, newNPC: NPC) {
	 ...
	 */
	private fun updateConversationHistoryNames(conversation: Conversation, oldName: String, newName: String) {
	 ...
	 */
	private fun generateContextualBioForNewNPC(newNpcData: NPCData, originalGenericData: NPCData) {
		try {
			...
			val loreContexts = originalGenericData.nameBank?.let { nameBank ->
					...
					Runnable {
						if (response != null) {
							try {
								...
								plugin.logger.warning("Failed to parse AI response for NPC bio generation: ${e.message}")
							}
						} else {
