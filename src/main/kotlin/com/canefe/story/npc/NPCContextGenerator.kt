...
class NPCContextGenerator(private val plugin: Story) {
	...
	fun getOrCreateContextForNPC(npc: NPC): NPCContext? {
		return getOrCreateContextForNPCInternal(npc.name, npc.uniqueId.toString())
	}
	...
	fun getOrCreateContextForNPC(npcName: String): NPCContext? {
		return getOrCreateContextForNPCInternal(npcName, null)
	}

	private fun getOrCreateContextForNPCInternal(npcName: String, actualEntityId: String?): NPCContext? {
		try {
			...
			val actualName = if (npcData?.generic == true && npcData.nameBank != null) {
				// Use name resolver to get or generate canonical name
				val canonicalName = plugin.npcNameResolver.resolveCanonicalName(
					displayName = npcName,
				...
				if (canonicalName != npcName && npcData.canonicalName != canonicalName) {
					...
					for (conversation in activeConversations) {
						...
						if (originalNPC != null) {
							...
							val alias = plugin.npcNameManager.getOrCreateAlias(
			...
			val finalContext = if (finalNpcData.generic) {
			...
			val memories = if (finalNpcData.generic) {
				emptyList<Memory>()
			} else {
				finalNpcData.memory
			}
