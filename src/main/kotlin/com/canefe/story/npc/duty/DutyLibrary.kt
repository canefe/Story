 ...
 */
class DutyLibrary private constructor(private val plugin: Story) {
	...
	fun loadLocationDutyData(location: StoryLocation): LocationDutyData {
		...
		if (dutyFile?.exists() != true) {
			...
			val emptyData = LocationDutyData(
	 ...
	 */
	fun getDutyScript(location: StoryLocation, scriptName: String): DutyScript? {
	 ...
	 */
	fun getWorkstationLocation(location: StoryLocation, workstationName: String): Location? {
		val dutyData = loadLocationDutyData(location)
	 ...
	 * Get bark pool
	 */
	fun getBarkPool(location: StoryLocation, poolName: String): BarkPool? {
		val dutyData = loadLocationDutyData(location)
