...
class NPCScheduleManager private constructor(private val plugin: Story) {
				...
				override fun run() {
					...
					if (timeSinceReload < 3000) {
						if (plugin.config.debugMessages) {
							plugin.logger.info("Skipping schedule processing - recently reloaded (${timeSinceReload}ms ago)")
						}
						return
					}
					...
					if (plugin.config.randomPathingEnabled) {
							...
							if (lastProcessedTime != null) {
								...
								if (timeSinceLastProcessed < cooldownMs) {
									if (debugMessages) {
										...
										plugin.logger.info("NPC ${npc.name} is in cooldown for ${remainingSeconds}s, skipping.")
									}
									return@filter false
								}
							}
						...
						if (debugMessages) {
							...
							val cooldownCount = nearbyNPCs.count { npc ->
								val lastProcessedTime = npcRandomPathingCooldowns[npc.name.lowercase()]
								if (lastProcessedTime != null) {
									val cooldownMs = plugin.config.randomPathingCooldown * 1000L
						...
						if (candidateNPCs.isNotEmpty()) {
							...
							if (plugin.config.randomPathingEnabled) {
								...
								for (i in 0 until Math.min(numToSelect, shuffledCandidates.size)) {
									...
									if (debugMessages) {
										plugin.logger.info("Added ${npc.name} to movement queue (${i + 1}/$numToSelect)")
									}
								}
								...
								for (npc in queueSnapshot) {
									...
									if (delayTicks == 0L) {
										...
										if (debugMessages) {
											plugin.logger.info("Processing NPC ${npc.name} from queue immediately (${processed + 1}/$maxProcessPerTick)")
										}
									} else {
										...
										Bukkit.getScheduler().runTaskLater(
											...
											Runnable {
												...
												npcRandomPathingCooldowns[npc.name.lowercase()] = System.currentTimeMillis()
												if (debugMessages) {
													plugin.logger.info("Processing NPC ${npc.name} from queue with $delayTicks tick delay")
												}
											},
											delayTicks,
										)
	...
	private fun cleanupExpiredDialogueCooldowns() {
		...
		val expiredEntries = npcLastDialogueTimes.entries.filter { (_, timestamp) ->
	...
	private fun moveNPCToRandomSublocation(npc: NPC) {
		...
		val randomSublocation = synchronized(locationOccupancy) {
			// Filter out locations that are occupied by actions that would conflict inside the synchronized block
			val currentlyEligibleLocations = eligibleLocations.filter { location ->
				...
				if (action != null && (action.lowercase() == "sit" || action.lowercase() == "sleep")) {
					...
					if (debugMessages) {
						plugin.logger.info("SYNC: Checking location ${location.name} for ${npc.name}: occupied = $isOccupied")
						if (isOccupied) {
							val occupancy = locationOccupancy["${location.name}:$action"]
							plugin.logger.info("SYNC: Location ${location.name} is occupied by ${occupancy?.npcName}")
						}
					}
					!isOccupied
				} else {
			...
			if (debugMessages) {
				plugin.logger.info(
					"SYNC: Currently eligible locations for ${npc.name}: ${currentlyEligibleLocations.joinToString(", ") { it.name }}",
				)
			}
			...
			if (currentlyEligibleLocations.isEmpty()) {
				...
				if (locationAction != null && (locationAction.lowercase() == "sit" || locationAction.lowercase() == "sleep")) {
		...
		val targetLocation = if (requiresPrecisePositioning) {
		...
		randomSublocation.randomPathingAction?.let { action ->
			...
			Bukkit.getScheduler().runTaskLater(
				...
				Runnable {
					...
					if (plugin.config.debugMessages) {
						plugin.logger.info("Executed random pathing action '$action' for ${npc.name} at ${randomSublocation.name}")
					}
				},
				20L, // 1 second delay
			)
							...
							if (!entry.dialogue.isNullOrEmpty()) {
								...
								if (isNPCInDialogueCooldown(npc.name)) {
									if (plugin.config.debugMessages) {
										...
										plugin.logger.info("${npc.name} is in dialogue cooldown for ${remainingSeconds}s, skipping dialogue")
									}
									return@Runnable
								}
	...
	private fun executeAction(npc: NPC, action: String, location: StoryLocation? = null) {
	 ...
	 */
	fun isScheduleRunnerActive(): Boolean {
	...
	private fun cleanupExpiredCooldowns() {
		...
		val expiredEntries = npcRandomPathingCooldowns.entries.filter { (_, timestamp) ->
			currentTime - timestamp > cooldownMs * 2 // Remove entries that are more than 2x the cooldown period old
		}
	 ...
	 */
	private fun isLocationOccupied(location: StoryLocation, action: String): Boolean {
		...
		val isStillPerformingAction = when (action.lowercase()) {
			"sit" -> occupyingNPC.getOrAddTrait(SitTrait::class.java).isSitting
			"sleep" -> occupyingNPC.getOrAddTrait(EntityPoseTrait::class.java).pose == EntityPoseTrait.EntityPose.SLEEPING
	 ...
	 */
	private fun markLocationOccupied(location: StoryLocation, npcName: String, action: String) {
		val locationKey = "${location.name}:$action"
	 ...
	 */
	private fun clearLocationOccupancy(location: StoryLocation, action: String) {
		val locationKey = "${location.name}:$action"
		val removed = locationOccupancy.remove(locationKey)

		if (removed != null && plugin.config.debugMessages) {
			plugin.logger.info("Cleared occupancy for location '${location.name}' action '$action' (was ${removed.npcName})")
		}
	}
	 ...
	 */
	private fun clearNPCOccupancyExcept(npcName: String, exceptLocationKey: String) {
