...
class AIResponseService(private val plugin: Story) {
    ...
    fun getAIResponseStreaming(
        ...
        try {
            ...
            if (!apiRateLimiter.tryAcquire(1, TimeUnit.SECONDS)) {
                pendingRequests.decrementAndGet()
                plugin.logger.warning(
                    "API rate limit reached, couldn't acquire permit within timeout",
                )
                return null
            }
                        ...
                        if (line.startsWith("data: ")) {
                            ...
                            try {
                                ...
                                if (chunk.has("choices") &&
                                    chunk.getAsJsonArray("choices").size() > 0
                                ) {
                                ...
                                plugin.logger.fine(
                                    "Error parsing streaming response chunk: $jsonData",
                                )
    ...
    /**
     * Gets an AI response from OpenRouter.ai with improved concurrency handling This overloaded
     * version supports both streaming and non-streaming behavior
     *
     * @param conversation The conversation history to send
            ...
            try {
                ...
                httpClient.newCall(request).execute().use { response ->
                    ...
                    if (responseObject.has("choices") &&
                        responseObject.getAsJsonArray("choices").size() > 0
                    ) {
