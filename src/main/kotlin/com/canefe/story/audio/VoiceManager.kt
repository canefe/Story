 ...
 */
class VoiceManager(private val plugin: Story) {
	 ...
	 */
	fun generateSpeechForNPC(npc: NPC, message: String, players: HashSet<Player>): CompletableFuture<Boolean> {
		...
		return audioManager.generateSpeechOnce(cleanMessage, voiceId, npc.name)
			.thenApply { audioData ->
				if (audioData != null) {
					...
					plugin.logger.info("Successfully sent audio to $successCount/${players.size} players for NPC ${npc.name}")
					successCount > 0
				} else {
	 ...
	 * @return CompletableFuture indicating success/failure
	 */
	fun generateSpeechForPlayer(player: Player, message: String, players: HashSet<Player>): CompletableFuture<Boolean> {
		...
		// Generate speech once and send to all players
		return audioManager.generateSpeechOnce(cleanMessage, voiceId, player.name)
			.thenApply { audioData ->
				if (audioData != null) {
					// Send the same audio data to all players
					var successCount = 0
					...
					plugin.logger.info("Successfully sent audio to $successCount/${players.size} players for player ${player.name}")
					successCount > 0
				} else {
	 ...
	 */
	fun generateSpeechForSinglePlayer(npc: NPC, message: String, player: Player): CompletableFuture<Boolean> {
		// Skip if voice features are disabled
		if (!isEnabled()) {
			return CompletableFuture.completedFuture(false)
		}
	 ...
	 */
	fun setNPCVoice(npcName: String, voiceId: String) {
