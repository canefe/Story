 ...
 */
class MMOCoreSkillProvider(private val characterId: UUID, private val isPlayer: Boolean) : SkillProvider {
	...
	override fun getSkillLevel(skillName: String): Int {
		...
		if (!org.bukkit.Bukkit.getPluginManager().isPluginEnabled("MMOCore")) {
			return 0
		}
	...
	override fun getAllSkills(): List<String> {
		if (!isPlayer || !org.bukkit.Bukkit.getPluginManager().isPluginEnabled("MMOCore")) {
			return emptyList()
		}
		...
		return MMOCore.plugin.professionManager.getAll().map { it.name }
	}
