...
data class CharacterData(
	 ...
	 */
	private fun checkItemForSkillModifiers(item: ItemStack, pattern: Pattern) {
	...
	companion object {
		 ...
		 */
		fun fromNPCData(id: UUID, npcData: NPCData): CharacterData = CharacterData(
			id = id,
			name = npcData.name,
			role = npcData.role,
			...
			// Convert memories
			this.memory = npcData.memory.map { npcMemory ->
				Memory(
					id = npcMemory.id,
		 ...
		 */
		fun fromPlayer(player: Player): CharacterData = CharacterData(
			id = player.uniqueId,
			name = player.name,
			role = "Player",
