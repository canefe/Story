...
data class LocationContextInfo(val name: String, val context: String)

/** Data class for NPC context information */
data class NPCContextInfo(val name: String, val context: String, val recentMemories: String = "")
 ...
 */
class ContextExtractor(private val plugin: Story) {
	...
	private fun extractLoreContexts(text: String, config: ContextExtractionConfig): List<LoreContext> {
		return plugin.lorebookManager.findLoresByKeywords(text).take(config.maxLoreContexts)
	}

	/** Extract relevant location contexts from text */
	private fun extractLocationContexts(text: String, config: ContextExtractionConfig): List<LocationContextInfo> {
	...
	private fun extractNPCContexts(text: String, config: ContextExtractionConfig): List<NPCContextInfo> {
	...
	private fun extractKeywords(text: String, minLength: Int = 3): List<String> {
		return text.split(" ")
