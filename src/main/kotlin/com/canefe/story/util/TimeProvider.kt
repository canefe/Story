...
class RealisticSeasonsTimeProvider : TimeProvider {
	...
	override fun getHours(): Int {
		return seasonsAPI!!.getHours(defaultWorld)
	}

	override fun getMinutes(): Int {
		return seasonsAPI!!.getMinutes(defaultWorld)
	}

	override fun getSeason(): String {
		return seasonsAPI!!.getSeason(defaultWorld)?.toString() ?: "Unknown Season"
	}
}
...
class FallbackTimeProvider : TimeProvider {
	...
	override fun getHours(): Int {
		return (System.currentTimeMillis() / 1000 % 24).toInt()
	}
 ...
 */
class TimeService(private val plugin: Story) {
	...
	fun getCurrentGameTime(): Long {
		return timeProvider.getCurrentGameTime()
	}

	fun getFormattedDate(): String {
		return timeProvider.getFormattedDate()
	}

	fun getHours(): Int {
		return timeProvider.getHours()
	}

	fun getMinutes(): Int {
		return timeProvider.getMinutes()
	}

	fun getSeason(): String {
		return timeProvider.getSeason()
	}
