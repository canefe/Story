...
object EssentialsUtils {
	...
	fun getLastLoginTime(player: String): Long? {
		return getUserYaml(player)?.getLong("timestamps.login")
	}

	fun getLastLogoutTime(player: String): Long? {
		return getUserYaml(player)?.getLong("timestamps.logout")
	}

	fun getLastTeleportTime(player: String): Long? {
	...
	fun hasSocialSpy(player: String): Boolean? {
		return getUserYaml(player)?.getBoolean("socialspy")
	}

	fun isAFK(player: String): Boolean? {
	...
	fun getIP(player: String): String {
