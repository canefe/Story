...
data class Settlement(
	...
	fun convertChestToMoney(chest: Block): BigDecimal {
		...
		inventory.contents.forEach { item ->
			...
			for (entry in denominations) {
				...
				val denominationMaterial = (denomination as? ItemDenomination)?.material?.replace("minecraft:", "") ?: continue
	...
	private fun broadcastMineResults(
		...
		val message2 =
			"${ChatColor.GRAY}⛏ ${ChatColor.GRAY}wages -${ChatColor.RED}₿${expenses.setScale(2, RoundingMode.HALF_UP)} " +
	...
	fun getStatusDisplay(): List<String> {
		val result = mutableListOf<String>()
		result.add("${ChatColor.GOLD}=== ${ChatColor.YELLOW}$name ${if (isCapital) "(Capital)" else ""} ${ChatColor.GOLD}===")
