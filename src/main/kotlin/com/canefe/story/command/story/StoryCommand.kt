        ...
        return CommandAPICommand("message")
            ...
            .executes(
                CommandExecutor { sender, args ->
                    ...
                    val contextExtractor = ContextExtractor(plugin)
                    ...
                    val systemPrompt =
                        plugin.promptService.getStoryMessageGenerationPrompt(
                            contextInformation,
                        )

                    prompt.add(ConversationMessage("system", systemPrompt))
                    ...
                    plugin.getAIResponse(prompt)
                        .thenAccept { response ->
                            ...
                            if (targetPlayer != null) {
                                ...
                                sender.sendSuccess(
                                    "Message sent to ${targetPlayer.name}.",
                                )
                            } else {
        ...
        return CommandAPICommand("gm")
            ...
            .executes(
                CommandExecutor { sender, args ->
                    ...
                    // Extract context using the centralized ContextExtractor
                    val contextExtractor = ContextExtractor(plugin)
                    val extractedContext = contextExtractor.extractContext(question)
                    ...
                    plugin.locationManager.getAllLocations().forEach { location ->
                        ...
                        // Special check for multi-part paths to prevent matching just
                        // "Fort"
                    ...
                    // Build the system prompt using prompt service and extracted context
                    // For GM command, we need to integrate the complex location logic with
                    // extracted context
                    ...
                    val baseContext = extractedContext.generatePromptContext()
                    val additionalLocationContext =
                        if (combinedLocations.isNotEmpty() &&
                            combinedLocationContexts.isNotEmpty()
                        ) {
                            val newLocationContexts =
                                combinedLocationContexts.filter { ctx ->
                                    !baseContext.contains(ctx.substringBefore(":"))
                                }
                            if (newLocationContexts.isNotEmpty()) {
                                "\n\nAdditional location information:\n${
                                    newLocationContexts.joinToString(
                                        "\n",
                                    )
                                }"
                            } else {
                    ...
                    val contextInformation = baseContext + additionalLocationContext
                    val systemPrompt =
                        plugin.promptService.getGameMasterResponsePrompt(
                            contextInformation,
                        )
                    ...
                    plugin.getAIResponse(prompts)
                        .thenAccept { response ->
                            if (response.isNullOrEmpty()) {
                                sender.sendSuccess(
                                    "I couldn't formulate a response to your question. Please try asking differently.",
                                )
                                return@thenAccept
                            }
            ...
            .executes(
                CommandExecutor { sender, _ ->
                    ...
                    tasks.forEach { task ->
                        ...
                        sender.sendRaw(
                            "<white>Task #${task.id}</white>: ${task.description}$timeoutInfo",
                        )
        ...
        for (paragraph in paragraphs) {
            if (paragraph.length <= maxLength) {
                ...
                for (sentence in sentences) {
                    val potentialPart =
                        if (currentPart.isEmpty()) sentence else "$currentPart. $sentence"
